function ATrackingSCRIPT_func(PathName,FileName,SaveName)
% renamed from "ATrackingSCRIPT_CKscripted.m [10/3/2019]
...
fps=29.79;                %Frames per second of capture
ConversionFactor = 6.5/30; %1 for pixel units, otherwise use pixel to um conversion from scope with magnification (use microscope booking site for camera spec)
...
% File Definition
FileStruct  = FileFind(PathName,FileName,'*.*','none');     %No arguments for GUI, can also pass a path and filename to convert to correct structure.  Choose a file with the same format as the others of interest
%FileStruct  = FileFind();
FileNumbers = [43:299];        %The numbers in the filenames of the images to be processed

% File save name and path.  Modify if desired.
SaveName='TrackingResults_Rep10025.mat'; % scripted in the for-loop in RunATrackingSCRIPT
...
%ImageNumber=FileNumbers();
% To refine? the filter parameters, use:originally had [6,.5,15], for rean
% it was 4,.8,25
%FilterTest(FileStruct,ImageNumber,{[6,.7,25],'gaussian'}) 
% Run the FilterTest and determine the best parameters for each set

FilterParameters = {[6,.7,25],'gaussian'}; %Parameters for spatial filtering:
...
Threshold =  [-27.3364,-1];%(was -62.2084, -1)[] for GUI, make array with second value +/- 1 to indicate direction of threshold, default positive.
...
UpperBound =-1000; %Upper bound for restricted field, or Inf
...
UpperBound = 4; %Upper bound for restricted field, or Inf
...
DistanceLimit = 6;            % Limit of distance a particle can travel between frames, in units defined by ConversionFactor
...
MinFrameLength=3;  %Set the minimum number of frames for a track of interest, needs to be at least > 2*FitLength
...
FitLength=1;    %This describes the type and size of fitting for calculation, 
...
%figure(7); PlotTrackStats(P_Tracks_Analysis) 

% Plot the Tracks & overlay velocity
%figure(8); PlotTracks(P_Tracks_Analysis)
...
%figure(9); clf; PlotParticleVel(PTA)

...
# UPDATED (to _02.R) : 9/7/2020 (compatible with cross-referencing with 0 min results)
#-----------------------------
...
save_dir='/Users/Cherry/Dropbox (MIT)/Lab/Research Files (Dropbox)/02_Vcor paper/CK-transcriptomics_data/TRANSCRIPTOME_ANALYSIS/Ranal/virulence_genes/heatmap_log2fc/cross_ref_0min_results' # where to save timelapse plots
  # dir.create(save_dir)
...
res0_match_10 = data_concat_10$main_base_results_match[gene_idx_10_ordered]
  data_10 = cbind(10, fc_10, padj_10, as.character(gene_name_list), c(1:length(gene_name_list)), res0_match_10)
...
res0_match_60 = data_concat_60$main_base_results_match[gene_idx_60_ordered]
  data_60 = cbind(60, fc_60, padj_60, as.character(gene_name_list), c(1:length(gene_name_list)),res0_match_60)
...
min0_match_id = which(data_plot$res0_match == 'yes')
...
g <- g + annotate("text", label=" *", x = data_plot$order[s_index], y=data_plot$time[s_index], size = 5) + 
         annotate("text", label=" x", x = data_plot$order[min0_match_id], y=data_plot$time[min0_match_id]+10, size = 4)
...
g <- g + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),                      # get rid of grey background
         ...
         labs(title = paste('Heatmap: log2 fold change of ',geneset_description, 'genes'),
              subtitle = paste('x = match ID with 0; * = significant \n color scaled on all (n.s. and s.)'))
